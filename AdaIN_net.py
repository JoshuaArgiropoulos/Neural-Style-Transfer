...
class AdaIN_net(nn.Module):
    ...
    def forward(self, content, style, alpha=1.0):
        ...
        if self.training:  # training
            ...
            content_feat = self.encode(content)[-1]#Content features
            style_feats = self.encode(style) #Style Features
            ...
            #
            #   your code here ...
            # Encode the content image to get content features
            content_feat = self.encode(content)[-1]
            
            # Encode the style image to get style features
            style_feats = self.encode(style)

            # Perform Adaptive Instance Normalization (AdaIN) to transfer style
            t = self.adain(content_feat, style_feats[-1])
            
            t = alpha * t + (1 - alpha) * content_feat

            # Generate an output image from the transformed features
            output_image = self.decode(t)
